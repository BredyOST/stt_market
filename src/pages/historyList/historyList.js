        ...
        if (props.fullHistory.length > 0) {
            ...
            for (const ctrResult of new_hash_list.slice(start_length, end_length)) {
                if (ctrResult['type'] === 'ЗАПРОС ПЕРЕВОДА') {
                    ...
                    try {
                        ...
                        if (decoded_recipient !== acc || ctrResult['type'] === 'Transfer In') {
                            ...
                            if ('amount' in ctrResult) {
                                ...
                                decoded = parseFloat(ethers.AbiCoder.defaultAbiCoder().decode(['uint'], transaction.logs.slice(-1)[0].data).toString()) / Math.pow(10, 9)
                            }
                            let timestamp = (await current_provider.getBlock(transaction.blockNumber)).timestamp * 1000
                            let time = new Date(timestamp);
                            let date = time.toLocaleDateString("en-GB", { // you can use undefined as first argument
                                year: "numeric",
                                month: "2-digit",
                                day: "2-digit",
                            })
                            let date_full = time.toLocaleDateString("en-GB", { // you can use undefined as first argument
                                year: "numeric",
                                month: "2-digit",
                                day: "2-digit",
                                hour: "2-digit",
                            ...
                            if (transaction.to === ethers.getAddress(exchangeContractAddress)) {
                                ...
                                for (let i = 1; i <= transaction.logs.length - 1; i++) {
                                    ...
                                    if (ethers.getAddress(decoded_recipient) !== ethers.getAddress(exchangeContractAddress) && ethers.getAddress(decoded_recipient) !== ethers.getAddress(acc)) {
                                        let decoded = parseFloat(ethers.AbiCoder.defaultAbiCoder().decode(['uint'], transaction.logs[i].data).toString()) / Math.pow(10, 9)
                                        sum_result += decoded
                                    }
                                }
                    ...
                    let date_usdt = time_usdt.toLocaleDateString("en-GB", { // you can use undefined as first argument
                        year: "numeric",
                        month: "2-digit",
                        day: "2-digit",
                    })
                    let date_full_usdt = time_usdt.toLocaleDateString("en-GB", { // you can use undefined as first argument
                        year: "numeric",
                        month: "2-digit",
                        day: "2-digit",
                        hour: "2-digit",
                    ...
                    if (ctrResult['status'] === 'full') {
                        ...
                        let date_stt = time_stt.toLocaleDateString("en-GB", { // you can use undefined as first argument
                            year: "numeric",
                            month: "2-digit",
                            day: "2-digit",
                        })
                        let date_full_stt = time_stt.toLocaleDateString("en-GB", { // you can use undefined as first argument
                            year: "numeric",
                            month: "2-digit",
                            day: "2-digit",
                            hour: "2-digit",
    ...
    return (
        <>
            {historyLoading
                ...
                : <>
                    <ScrollContainer style={{display: 'block', overflowX: 'auto', paddingTop: 25}} className={"history_wrapper"} id={"scrollable-history-wrapper"} onEndScroll={() => setScrollPosition()}>
                        <div className={"col-12 partners"} style={{width: `${ historyWidth }px`, margin: 'auto', height: 250}}>
                            {
                                s_history.map((item, i) => (
                                    <HistoryItem key={i} cat_title={item['cat_title']} img_url={item['img_url']} time={item['time']} full_time={item['full_time']} type={item['type']} modifier={item['modifier']} amount={item['amount']} class={item['class']} txhash={item['txHash']} item={item} img_type={item['img_type']} />
                                ))
                            }
                            {s_history.length === 0
                                ? <>
                                    {props.historyNeedLoad
                                        ...
                                        : <div className={"history_card"} style={{cursor:"default", backgroundColor: 'transparent', justifyContent: "center", width: 200, paddingTop: 70, height: 75}}>
                                            <i className="fa-light fa-receipt"></i>
                                        </div>
                                    }
                                </>
