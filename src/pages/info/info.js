...
function Info(props) {
    ...
    function handleBalances() {
        if (!props.withoutWallet) {
            ...
            axios
                .get(
                    'https://api.arbiscan.io/api?module=stats&action=tokensupply&contractaddress=0x1635b6413d900D85fE45C2541342658F4E982185&apikey=2U5WS2IBB4WYBA99AJZ8P69YTD4BZJMURW'
                )
                .then((res) => {
                    setTotalSupply(
                        (res.data.result / Math.pow(10, 9))
                            .toFixed(2)
                            .toString()
                            .replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')
                            .replace('.', ',')
                    ...
                    setTimeout(function () {
                        axios
                            .get(
                                'https://api.arbiscan.io/api?module=account&action=tokenbalance&contractaddress=0xfd086bc7cd5c481dcc9c85ebe478a1c0b69fcbb9&address=0x88af31e521fca0ed362582aa231189849f1e3b2e&apikey=2U5WS2IBB4WYBA99AJZ8P69YTD4BZJMURW'
                            )
                            .then((res) => {
                                setContractBalance(
                                    (res.data.result / Math.pow(10, 6))
                                        .toFixed(2)
                                        .toString()
                                        .replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ')
                                        .replace('.', ',')
                                ...
                                setTimeout(function () {
                                    axios
                                        .get(
                                            'https://api.arbiscan.io/api?module=account&action=tokenbalance&contractaddress=0x1635b6413d900d85fe45c2541342658f4e982185&address=0x11F2754320C961f4fFbC0174C8B4587903F816Dc&apikey=2U5WS2IBB4WYBA99AJZ8P69YTD4BZJMURW'
                                        )
                                        .then((res) => {
                                            setMultiBalance(
                                                (res.data.result / Math.pow(10, 9))
    ...
    return (
        <div className={'row'}>
            <div className={'col-12'}>
                <a
                    className={'balance_row eth-card'}
                    href={'https://arbiscan.io/token/0x1635b6413d900D85fE45C2541342658F4E982185'}
                    target={'_blank'}
                >
            ...
            <div className={'col-12'}>
                <a
                    className={'balance_row eth-card'}
                    href={'https://arbiscan.io/address/0x88af31e521fca0ed362582aa231189849f1e3b2e'}
                    target={'_blank'}
                >
            ...
            <div className={'col-12'}>
                <a
                    className={'balance_row eth-card'}
                    href={'https://arbiscan.io/address/0x11F2754320C961f4fFbC0174C8B4587903F816Dc'}
                    target={'_blank'}
                >
