...
function Insurance(props) {
    ...
    const [fullInStt, setFullInStt] = useState(0.0);
    const [fullBalance, setFullBalance] = useState(0.0);

    const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));

    async function handleRates() {
        if (props.account && !props.withoutWallet) {
                    ...
                    for (const con in extraTokenInfo) {
                        ...
                        tokenContract.balanceOf(newExchangeAddress).then((res) => {
                            ...
                            newExchangeContract.getPrice(extraTokenInfo[con].id).then((result) => {
                                extraBalances[con]['inStt'] = parseFloat(tokenBalance.toFixed(14)) / +(tokenBalance / totalSt).toFixed(14);
                                fullExtraInStt += parseFloat(tokenBalance.toFixed(14)) / +(tokenBalance / totalSt).toFixed(14);
                            });
                            setExtraBalances({ ...extraBalances });
                            const amount = parseFloat(tokenBalance.toFixed(14));
                            axios.get('https://api.diadata.org/v1/assetQuotation/Arbitrum/' + extraTokenInfo[con].contract).then((res) => {
                                const data = res.data;
                                const price = amount * parseFloat(data.Price);
                                fB = fB + price;
                                setFullBalance(fB);
                            });
                        });
                    }
                    setRate(fullUs / fullSt);
                    setFullBalance(fB);
                });
            });
        } else {
                ...
                axios
                    .get(
                        'https://api.arbiscan.io/api?module=account&action=tokenbalance&contractaddress=' +
                            extraTokenInfo[con].contract +
                            '&address=0xf5B4001A2aEe3821419576B1E9D96d160c69B901&apikey=2U5WS2IBB4WYBA99AJZ8P69YTD4BZJMURW'
                    )
                    .then((res) => {
                        const amount = (Number(res.data.result) / Math.pow(10, extraTokenInfo[con].decimals)).toFixed(14);
                        extraBalances[con] = {
                            id: extraTokenInfo[con].id,
                            name: extraTokenInfo[con].label,
                            icon: extraTokenInfo[con].icon,
        ...
        <React.Fragment>
            <div className={'main-block eth-card'}>
                <a
                    ...
                    href={'https://arbiscan.io/address/0xf5B4001A2aEe3821419576B1E9D96d160c69B901'}
                    target={'_blank'}
                >
                    INSURANCE
                    <img src={'/img/arrow-light.png'} alt={''} style={{ position: 'absolute', width: 35, top: 0, right: 5 }} />
                </a>
                <div className={'insurance_rows row'}>
                    {Object.keys(extraBalances).length > 0 ? (
                        <React.Fragment>
                            {Object.keys(extraBalances).map((item, key) => (
                                <Col xs={6} key={key}>
                                    <div className={'insurance-item'}>
                                        <div className={'insurance-item_icon'}>
                                            <img style={{ width: 36, padding: 4 }} src={extraBalances[item].icon} alt='' />
                                            <span className={'token-chip'}>
                                                <img src={arb_icon} alt={''} />
                                            </span>
                                        </div>
                                        <div className={'insurance-item_data'}>
                                            <div className={'insurance-item_data-title'}>{extraBalances[item].name}</div>
                                            <div className={'insurance-item_data-rate'}>{extraBalances[item].balance.toFixed(4)}</div>
                                        </div>
                                    </div>
                                </Col>
                            ))}
                ...
                <div className={'insurance_total'}>~ {fullBalance.toFixed(2)} $</div>
            </div>
        </React.Fragment>
    );
}
