        ...
        <div className={cls.wrapper}>
            ...
            <div className={cls.cover_info}>
                ...
                <div className={cls.timestamp}>
                    {new Date(transaction?.timestamp).getDay()}/{new Date(transaction?.timestamp).getMonth()}/
                    {new Date(transaction?.timestamp).getFullYear()} {new Date(transaction?.timestamp).getHours()}:
                    {new Date(transaction?.timestamp).getMinutes()}
                </div>
                <div className={cls.hash}>{transaction?.to.slice(26)}</div>
            </div>
            <CustomButton classNameBtn={cls.btn_ok} type='button' onClick={closeModalSwap}>
                ok
            </CustomButton>
        </div>
    );
